...
retcode_t json_get_trytes_serialize_request(const serializer_t* const s,
  ...
  ret = utarray_to_json_array(obj->hashes, json_root, "hashes");
  if (ret != RC_OK) {
    goto err;
  }
...
retcode_t json_get_trytes_deserialize_response(const serializer_t* const s,
  ...
  retcode_t ret = json_array_to_utarray(json_obj, "trytes", out->trytes);
  if (ret != RC_OK) {
    return ret;
  }

  out->numTransactions = utarray_len(out->trytes);
